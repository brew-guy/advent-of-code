  ...
  for bag in master:
    ...
    for content in before_replace:
      if content in master:
        ...
        print('{0} bag had {1} {2} => {3}'.format(bag, replaced[bag][content], content, new_content))
      t = [master[content] if content in master and master[content] != 'None' else content for content in replaced[bag]]
      replaced[bag] = [item for sublist in t for item in sublist]
    if before_replace == replaced[bag]:
      keep_replacing = False
      print('{0} changed from {1} to {2}'.format(bag, before_replace, replaced[bag]))
    else:
      keep_replacing = True
  iterations += 1
